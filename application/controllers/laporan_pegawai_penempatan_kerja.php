		...
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			else
			{
				if ($this->session->userdata('id_satuan_kerja') == "Semua" && $this->session->userdata('id_lokasi_kerja') == "Semua") {
					...
					$d['data_pegawai'] = $this->db->query("SELECT pegawai.nip, pegawai.nama_pegawai, pegawai.tanggal_lahir, pegawai.tanggal_lahir, pegawai.jenis_kelamin, pegawai.agama, pegawai.usia FROM tbl_data_pegawai as pegawai LEFT JOIN tbl_master_lokasi_kerja ON pegawai.lokasi_kerja = tbl_master_lokasi_kerja.id_lokasi_kerja LEFT JOIN tbl_master_satuan_kerja ON pegawai.id_satuan_kerja = tbl_master_satuan_kerja.id_satuan_kerja WHERE pegawai.id_satuan_kerja = '".$this->session->userdata('id_satuan_kerja')."'");
				} else {
					$d['data_pegawai'] = $this->db->query("SELECT pegawai.nip, pegawai.nama_pegawai, pegawai.tanggal_lahir, pegawai.tanggal_lahir, pegawai.jenis_kelamin, pegawai.agama, pegawai.usia FROM tbl_data_pegawai as pegawai LEFT JOIN tbl_master_lokasi_kerja ON pegawai.lokasi_kerja = tbl_master_lokasi_kerja.id_lokasi_kerja LEFT JOIN tbl_master_satuan_kerja ON pegawai.id_satuan_kerja = tbl_master_satuan_kerja.id_satuan_kerja WHERE pegawai.id_satuan_kerja = '".$this->session->userdata('id_satuan_kerja')."' AND pegawai.lokasi_kerja = '".$this->session->userdata('id_lokasi_kerja')."'");
				}
